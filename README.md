...
本项目仅作为一个轻量级的任务托管与调度引擎，本身不提供、不内置任何具有实际业务逻辑的脚本。所有在平台上运行的脚本及代码均需由用户自行添加或配置。

> **请注意：** 所有脚本的安全性均由**用户自行审核并负责**。由于脚本来源不可控，请在添加或执行任何第三方脚本之前仔细阅读审核其源代码，确保其安全性和合规性。因运行不安全、违规脚本带来的一切数据泄露、系统损坏及法律责任等后果与本项目无关，均由使用者自行承担。
